...
void Ellipse::ClassifyPointsHomeEllipse(MatL match_data, cv::Mat& classified_points, cv::Mat& motion_ratio, cv::Mat& maha_dist)
{
	...
	{
		classified_points.at<double>(i, 0) = match_data[idx_list[i]].at<double>(0, 9); // 9 current ref point x...
		classified_points.at<double>(i, 1) = match_data[idx_list[i]].at<double>(0, 10); // 10 current ref point y...
