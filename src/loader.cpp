...
Loader::Loader(int action_number, int trend_number, int dir_id)
{
	// initialize weights
	...
	sprintf(common_data_prefix_, "D:/Document/HKUST/Year 5/Research/Data/Arm Images/feb 22 2014/");
}
...
void Loader::AppendTrendName(char** trend_dir_str_array){
	if(trend_dir_str_array != NULL){
		...
		strcat(trend_dir_str_array[5], "cov_1");
		strcat(trend_dir_str_array[6], "cov_2");
		strcat(trend_dir_str_array[7], "idx");
	}
}
...
void Loader::LoadSiftKeyPoint(cv::Mat& descriptors, cv::Mat& key_points, int frame_idx)
{
	...
	FileIO::ReadMatFloat(data_w, 1, 1, input_dir);
	...
	FileIO::ReadMatFloat(data_h, 1, 1, input_dir);
	num_rows = (int)data_h.at<double>(0, 0); 
	num_cols = (int)data_w.at<double>(0, 0); 
	descriptors.create(num_rows, num_cols, CV_64F);
	key_points.create(num_rows, 2, CV_64F);
...
void Loader::LoadProprioception(int num_train_data, int num_test_data, cv::Mat& train_prop, cv::Mat& test_prop, cv::Mat& home_prop, cv::Mat& train_target_idx, cv::Mat& test_target_idx)
{
	...
	FileIO::ReadMatFloat(p_tmp_train, num_train_data, 1, input_dir);
	...
	FileIO::ReadMatFloat(p_tmp_test, num_test_data, 1, input_dir);
	...
	FileIO::ReadMatFloat(p_tmp_home, 2, 1, input_dir);
	...
	FileIO::ReadMatFloat(train_target_idx, num_train_data, 1, input_dir);
	...
	FileIO::ReadMatFloat(test_target_idx, num_test_data, 1, input_dir);
...
void Loader::RecordSiftKeyPoints()
{
	...
	int end_frame = 22607; // 23247; // 19228; // 11374;
	int image_width = 640;
	...
	for(int frame_idx = start_frame; frame_idx <= end_frame; frame_idx++)
	{
		...
		// mask = cv::Mat::zeros(curr_img.rows, curr_img.cols, CV_64F);
		// FileIO::ReadMatFloat(mask, mask.rows, mask.cols, "D:/Document/HKUST/Year 5/Research/Data/Arm Images/feb 22 2014/mask.bin");
		// mask.convertTo(mask, CV_8UC1);		
		//cv::imshow("mask", mask); // show ellipse
		//cv::waitKey(0);
		sift_detector.detect(curr_img, key_point_list); // , mask); // , evarBinary); // evDstBinaryCurr);
		...
		printf(tmp_dir, "descriptors/%d_pt.bin", frame_idx);
		strcpy(output_dir, common_data_prefix_);
		strcat(output_dir, tmp_dir);
		...
		printf(tmp_dir, "descriptors/%d.bin", frame_idx);
		strcpy(output_dir, common_data_prefix_);
		strcat(output_dir, tmp_dir);
